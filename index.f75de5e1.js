...
const sorting = (item)=>{
    const sortedRows = tbodyElements.sort((a, b)=>{
        switch(item){
            case "Name":
                return a.firstElementChild.textContent.localeCompare(b.firstElementChild.textContent);
            case "Position":
            ...
            case "Salary":
                const cellA = a.children[3].textContent.replace("$", "").replace(",", "");
                const cellB = b.children[3].textContent.replace("$", "").replace(",", "");
                return cellA - cellB;
            default:
                break;
        }
